...
Ci-dessus, la représentation graphique du chart "Hello World" proposé par défaut par Helm. À travers cette vue, nous identifions immédiatement que ce chart déploie trois ressources : un Service, un Deployment et un ServiceAccount. Les nœuds en vert représentent les helpers : des fonctions qui génèrent des fragments de configuration selon une logique pouvant elle-même dépendre des valeurs définies dans le fichier values.yaml.
...
Les graphes deviennent rapidement complexes pour des charts considérés comme modérés par l'industrie, comme le montre la figure ci-dessous représentant le chart ingress-nginx.
...
## Sous Questions, Hypothèses et Métriques
...
Chaque ressource a peu de dépendances (scope faible), mais elles pointent toutes vers le même noyau central (isolation faible). Le développeur peut comprendre une ressource en isolation, mais dès qu'il modifie l'un de ces helpers, il affecte toutes les ressources qui en dépendent. L'encapsulation cognitive (ce que je dois comprendre) est bonne ; l'encapsulation fonctionnelle (ce que je risque d'affecter) est mauvaise.

**SQ3 : Débogage. Hypothèse H3 partiellement validée**

Le débogage présente un tableau contrasté. Le Cognitive Diameter faible (2.83) facilite le traçage des dépendances : la cause d'une erreur n'est jamais très éloignée de sa manifestation, généralement à 2-3 hops maximum. Un développeur qui constate un problème sur un Service peut raisonnablement explorer ses dépendances immédiates sans se perdre dans une chaîne interminable.
