...
All variables that have become obsolete after the engineering of new features are added to a vector. All variables in this vector are dropped at once. In the next version, I will experiment with taking out variables that double count square feet measures. 
...
**Skewness**
Skewness is a measure of the symmetry in a distribution.  A symmetrical dataset will have a skewness equal to 0.  So, a normal distribution will have a skewness of 0. Skewness essentially measures the relative size of the two tails. As a rule of thumb, skewness should be between -1 and 1. In this range, data are considered fairly symmetrical. In order to fix the skewness, I am taking the log for all numeric predictors with an absolute skew greater than 1 (actually: log+1, to avoid division by zero issues).
...
By default, a predictor is classified as near-zero variance if the percentage of unique values in the samples is less than 10% and when the frequency ratio is greater than 95/5 (=19). In this case, this means that many categorical factor levels with few observations (few ones), are currently taken out. For instance all Neighborhoods with fewer than 146 houses ((2917-146)/146=19) are removed as (one hot encoded) variables.
...
Lasso regression works well in situations with multicolinearity. It selects only one variable if it comes across multicolinearity (for instance GrLivArea and TotRmsAbvGrd). Therefore, I consider it a good starting point.
...
In upcoming versions, I will add hyper parameter tuning for this model and add more models (Ridge, Elastic Net, XGBoost).
