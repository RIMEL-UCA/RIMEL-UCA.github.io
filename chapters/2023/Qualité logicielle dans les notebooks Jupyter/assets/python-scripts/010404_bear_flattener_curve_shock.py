...
bear_flattener_scenario = CurveScenario(market_data_pattern=MarketDataPattern('IR', 'USD'),
                                        curve_shift=-10, tenor_start=15, tenor_end=25, pivot_point=25)
...
print('Price under bear flattener curve shift: {:,.2f}'.format(swaption_bear_flattener))
...
market_data_df = pd.DataFrame([{mkt_data: value * 1e4 for mkt_data, value in market_data.items() if (mkt_data.mkt_type=="IR" and mkt_data.mkt_asset=="USD" and mkt_data.mkt_class=="SWAP OIS")},
                               {mkt_data: value * 1e4 for mkt_data, value in market_data_bear_flattener.items() if (mkt_data.mkt_type=="IR" and mkt_data.mkt_asset=="USD" and mkt_data.mkt_class=="SWAP OIS")}],
...
swap_curve.plot(figsize=(12, 8), title='USD Swap Curve Before and After {}bp Bear flattening Shock'.format(bear_flattener_scenario.curve_shift))
