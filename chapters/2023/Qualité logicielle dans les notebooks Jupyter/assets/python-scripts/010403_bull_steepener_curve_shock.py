...
bull_steepener_scenario = CurveScenario(market_data_pattern=MarketDataPattern('IR', 'USD'),
...
market_data_df = pd.DataFrame([{mkt_data: value * 1e4 for mkt_data, value in market_data.items() if (mkt_data.mkt_type=="IR" and mkt_data.mkt_asset=="USD" and mkt_data.mkt_class=="SWAP OIS")},
                               {mkt_data: value * 1e4 for mkt_data, value in market_data_bull_steepener.items() if (mkt_data.mkt_type=="IR" and mkt_data.mkt_asset=="USD" and mkt_data.mkt_class=="SWAP OIS")}],
...
swap_curve.plot(figsize=(12, 8), title='USD Swap Curve Before and After {}bp bull Steepening Shock'.format(bull_steepener_scenario.curve_shift))
